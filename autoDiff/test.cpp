...
namespace SAD {  // Simple Automatic Differentiation
    class ADV {
        public:
            ...
            // overloaded unary and binary operation
        ...
        //private:
            ...
            double dval; // derivatives of the variable
    };
    ...
    ADV sin(const ADV &x) {
        ADV y;
        y.val = std::sin(x.val);
        y.dval = std::cos(x.val) * x.dval; // y = sin(x), chain rule
        return y;
    }
    ...
    ADV cos(const ADV &x) {
        ADV y;
        y.val = std::cos(x.val);
        y.dval = -std::sin(x.val) * x.dval; // y = cos(x), chain rule
        return y;
    }
}
